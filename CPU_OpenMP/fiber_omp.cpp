...
void compute()
{
	...
	#pragma omp parallel for num_threads(ThreadNum) private(i,j,k,data,iq,iqz,fj,fk) shared(span, linenum)
	for(i = 0 ; i < span; i++)
	{
		data = q[i];
	    iq = iqz = 0.f;	
	...
	printf("Elapsed time for compute intensity = %lf(s)\n", end-start);
